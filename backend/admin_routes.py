        ...
        if not existing or not (existing.get('pix_key') or '').strip():
            await _db.settings.update_one(
                ...
                {'$set': {
                    ...
                    'pix_nome': 'CONCURSO ALAGOAS',
                    'pix_cidade': 'MACEIO AL',
                }},
                upsert=True,
            )
            logger.info("Seeded default PIX config (danielmmm950@gmail.com / CONCURSO ALAGOAS / MACEIO AL)")
    except Exception as e:
...
async def generate_pix_brcode(payload: Dict[str, Any]):
    ...
    nome = (s.get('pix_nome') or 'CONCURSO ALAGOAS').upper()
    cidade = (s.get('pix_cidade') or 'MACEIO AL').upper()
...
def _build_telegram_message(insc: Dict[str, Any], settings: Dict[str, Any] = None) -> str:
    ...
    titulo = settings.get('telegram_titulo') or 'NOVA INSCRIÇÃO - SESAU AL 26'
